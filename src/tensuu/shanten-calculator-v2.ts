...
/**
 * 統一バックトラッキングによるシャンテン計算
 */
export class UnifiedShantenCalculator {
  private minShanten: number = 8;
  private optimalStates: BacktrackState[] = [];
  private debugMode: boolean = false;

  /**
   * 通常手のシャンテン数を計算（統一バックトラッキング版）
   */
  public calculateRegularShanten(tileCount: TileCount, meldCount: number = 0, debugMode: boolean = false): number {
    this.minShanten = 8;
    this.optimalStates = [];
  ...
  private backtrack(tiles: TileCount, position: number, state: BacktrackState): void {
    ...
    if (position < 27 && this.canFormShuntsu(position) && state.mentsuCount < 4) {
      const canForm = tiles.getCount(position) >= 1 && 
                     tiles.getCount(position + 1) >= 1 && 
                     tiles.getCount(position + 2) >= 1;
      
      if (canForm) {
        tiles.decrement(position);
        tiles.decrement(position + 1);
        ...
        state.components.pop();
        state.mentsuCount--;
        tiles.increment(position);
        tiles.increment(position + 1);
    ...
    if (position < 27 && this.canFormTaatsu(position)) {
      if (tiles.getCount(position) >= 1 && tiles.getCount(position + 1) >= 1) {
        tiles.decrement(position);
        tiles.decrement(position + 1);
        state.taatsuCount++;
    ...
    if (position < 27 && this.canFormKanchan(position)) {
      if (tiles.getCount(position) >= 1 && tiles.getCount(position + 2) >= 1) {
        tiles.decrement(position);
        tiles.decrement(position + 2);
        state.taatsuCount++;
   ...
   * デバッグ情報の取得
   */
  public getOptimalStates(): BacktrackState[] {
    return this.optimalStates;
  }
  ...
  private outputDebugResults(): void {
    ...
    console.log(`最適パターン数: ${this.optimalStates.length}`);
